...
function notifyAndWhatsApp(p,waUrl,eventName,eventId){fetch("/api/notify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({nid:p.nid,titulo:p.titulo||"",ubicacion:[p.barrio,p.conjunto,p.ciudad].filter(Boolean).join(", "),precio:fmt(p.precio_venta),area:p.area||"N/A",habitaciones:p.habitaciones||"N/A",banos:p.banos||"N/A",bonoHabi:p.bonoHabi?fmt(p.bonoHabi):"",eventName:eventName||"Contact",eventId:eventId||"",sourceUrl:typeof window!=="undefined"?window.location.href:"",fbp:typeof document!=="undefined"?(document.cookie.match(/(?:^|;\s*)_fbp=([^;]*)/)||[])[1]||"":"",fbc:typeof document!=="undefined"?(document.cookie.match(/(?:^|;\s*)_fbc=([^;]*)/)||[])[1]||"":""})}).catch(()=>{});window.open(waUrl,"_blank")}
function handleWhatsAppClick(p){const eid=trackContact(p);const url=typeof window!=="undefined"?window.location.href:"";const msg=num(p.bonoHabi)>0?`Hola, vi este apto ${url} y quiero saber si aplica para el Bono Habi`:`Hola, vi este apto ${url} y me gustaría tener más información`;const waUrl="https://wa.me/"+WA+"?text="+encodeURIComponent(msg);notifyAndWhatsApp(p,waUrl,"Contact",eid)}
function handleScheduleClick(p){const eid=trackSchedule(p);const url=typeof window!=="undefined"?window.location.href:"";const msg=`Hola, quiero agendar una visita para el apto ${url}`;const waUrl="https://wa.me/"+WA+"?text="+encodeURIComponent(msg);notifyAndWhatsApp(p,waUrl,"Schedule",eid)}
