...
function handleWhatsAppClick(p){const url=typeof window!=="undefined"?window.location.href:"";const msg=num(p.bonoHabi)>0?`Hola, vi este apto ${url} y quiero saber si aplica para el Bono Habi`:`Hola, vi este apto ${url} y me gustaría tener más información`;const waUrl="https://wa.me/"+WA+"?text="+encodeURIComponent(msg);notifyAndWhatsApp(p,waUrl)}
  ...
  return(
    <div className="card-wrap" onClick={()=>onClick(p)} style={{background:"white",borderRadius:14,overflow:"hidden",cursor:"pointer",transition:"transform .2s, box-shadow .2s",boxShadow:"0 2px 12px rgba(27,79,114,0.07)",border:featured?"2px solid #FF6B35":"1px solid rgba(27,79,114,0.06)",minWidth:0}}>
      <div style={{position:"relative",paddingTop:"58%",overflow:"hidden",background:"#E8ECF0"}}>
        {realPhotos?<div key={ii} className={dir>0?"card-img-slide-right":"card-img-slide-left"}><Image src={imgs[ii]||"https://via.placeholder.com/600x400"} alt={p.titulo} fill sizes="(max-width:768px) 100vw, 280px" style={{objectFit:"cover"}} /></div>:p.url_360?<iframe src={p.url_360} title="Vista 360" loading="lazy" style={{position:"absolute",inset:0,width:"100%",height:"100%",border:"none",pointerEvents:"none"}} />:<div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,#1B2A4A,#2C3E50)",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg></div>}
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:"white",borderRadius:20,maxWidth:750,width:"100%",maxHeight:"95vh",overflow:"auto",boxShadow:"0 24px 80px rgba(0,0,0,0.3)",margin:"auto"}}>
        ...
        <div style={{padding:"clamp(14px,3vw,20px) clamp(14px,3vw,22px)"}}>
          ...
          <div className="modal-actions" style={{display:"flex",flexDirection:"column",gap:8,marginTop:16}}>
            <button onClick={()=>handleWhatsAppClick(p)} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,background:"linear-gradient(135deg,#1B4F72,#1B2A4A)",color:"white",borderRadius:12,padding:"14px 16px",fontSize:"clamp(13px,3vw,15px)",fontWeight:700,border:"none",cursor:"pointer",width:"100%",boxShadow:"0 4px 14px rgba(27,79,114,0.35)"}}>
